...
APPNAME=mtb-example-fx20-lvds-usb-stream
...
LPBK_EN?=no
ifeq ($(LPBK_EN), no)
    # Thread interleave can be enabled in this case.
    DEFINES += LVDS_LB_EN=0 PORT0_THREAD_INTLV=0
else
    # Thread interleave should not be enabled when loopback is used.
    DEFINES += LVDS_LB_EN=1 PORT0_THREAD_INTLV=0
endif
...
# Bind device with CyUSB3 driver when CYUSB=yes switch is used.
CYUSB?=no
ifeq ($(CYUSB), yes)
    DEFINES += USE_WINUSB=0
else
    DEFINES += USE_WINUSB=1
endif
...
ifeq ($(filter $(DEVICE),CYUSB4024-BZXI CYUSB4014-FCAXI CYUSB4013-FCAXI CYUSB4011-FCAXI CYUSB3084-FCAXI CYUSB3083-FCAXI CYUSB3284-FCAXI CYUSB4021-FCAXI),$(DEVICE))
    ...
    ifeq ($(filter $(DEVICE),CYUSB3081-FCAXI),$(DEVICE))
        DEFINES += CYFX_512K_RAM=1
        LDPATH=LinkerScripts/half_ram
    else
...
ifeq ($(CORE),CM4)
    ...
    $(error Unsupported CORE selection: $(CORE))
endif
